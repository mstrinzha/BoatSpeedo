...
void TM74HC595Display::digit4(int n, int replay, bool showZero)
{
  int n1[4];
  n1[0] = (int)  n % 10;
  n1[1] = (int) ((n % 100)-n1[0])/10;
  n1[2] = (int) ((n % 1000) - n1[1] - n1[0]) / 100;
  n1[3] = (int) ((n % 10000) - n1[2] - n1[1] - n1[0]) / 1000;
    
  //if(_dot >= 0 && _dot <=3 )
  //  n1[_dot] &= 0xBF;

 for(int i = 0; i<=replay; i++){
	send(_LED_0F[n1[0]] & (_dot == 0 ? 0x7F : 0xFF), 0b0001);
    if(showZero | n>9)send(_LED_0F[n1[1]] & (_dot == 1 ? 0x7F : 0xFF), 0b0010);
    if(showZero | n>99)send(_LED_0F[n1[2]] & (_dot == 2 ? 0x7F : 0xFF), 0b0100);
    if(showZero | n>999)send(_LED_0F[n1[3]] & (_dot == 3 ? 0x7F : 0xFF), 0b1000);
 }
}
...
void TM74HC595Display::dot(unsigned char pos)
{
    _dot = pos;
}